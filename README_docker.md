...
```bash
docker build -f ./.devcontainer/Dockerfile --tag=vulkan_rt:latest .
docker run -it vulkan_rt:latest
```
...
```bash
docker build --tag=vulkan_rt:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=vulkan_rt:latest --build-arg USE_CLANG=1 .
```

You will be logged in as root, so you will see the `#` symbol as your prompt.
You will be in a directory that contains a copy of `vulkan_rt`;
...
```bash
/vulkan_rt# mkdir build
/vulkan_rt# cmake -S . -B ./build
/vulkan_rt# cmake --build ./build
```
...
```bash
/vulkan_rt# mkdir build
/vulkan_rt# CC=clang CXX=clang++ cmake -S . -B ./build
/vulkan_rt# cmake --build ./build
```
