...
set(vulkan_rt_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(vulkan_rt_COI_WORKER "${vulkan_rt_WEB_DIR}/coi-serviceworker.min.js")
set(vulkan_rt_SHELL_TEMPLATE "${vulkan_rt_WEB_DIR}/shell_template.html.in")
set(vulkan_rt_INDEX_TEMPLATE "${vulkan_rt_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  # Set WASM build flag
  set(vulkan_rt_WASM_BUILD ON CACHE BOOL "Building for WebAssembly" FORCE)

  # Sanitizers don't work with Emscripten
  foreach(sanitizer ADDRESS LEAK UNDEFINED THREAD MEMORY)
    set(vulkan_rt_ENABLE_SANITIZER_${sanitizer} OFF CACHE BOOL "Not supported with Emscripten")
  endforeach()

  # Disable static analysis and strict warnings for Emscripten builds
  foreach(option CLANG_TIDY CPPCHECK WARNINGS_AS_ERRORS)
    set(vulkan_rt_ENABLE_${option} OFF CACHE BOOL "Disabled for Emscripten")
  endforeach()
  ...
  set(vulkan_rt_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(vulkan_rt_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(vulkan_rt_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(vulkan_rt_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY vulkan_rt_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE vulkan_rt_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${vulkan_rt_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${vulkan_rt_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${vulkan_rt_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${vulkan_rt_SHELL_TEMPLATE}")
      configure_file(
        "${vulkan_rt_SHELL_TEMPLATE}"
        "${CONFIGURED_SHELL}"
        @ONLY
      )
      ...
      message(FATAL_ERROR "Shell template not found: ${vulkan_rt_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${vulkan_rt_COI_WORKER}")
      add_custom_command(TARGET ${target} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
          "${vulkan_rt_COI_WORKER}"
...
function(vulkan_rt_create_web_dist)
  if(NOT EMSCRIPTEN)
    return()
  endif()
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY vulkan_rt_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${vulkan_rt_INDEX_TEMPLATE}")
    configure_file("${vulkan_rt_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${vulkan_rt_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY vulkan_rt_WASM_TARGET_${target}_OUTPUT_NAME)
