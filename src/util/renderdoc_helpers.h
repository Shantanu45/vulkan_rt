...
inline void init_render_doc(const std::string &rdoc_dir, const std::string &capture_dir)
{
  ...
  if (lib) {
    pRENDERDOC_GetAPI getAPI = (pRENDERDOC_GetAPI)GetProcAddress(lib, "RENDERDOC_GetAPI");
    if (getAPI) {
      int ret = getAPI(eRENDERDOC_API_Version_1_7_0, (void **)&rdoc);
      DEBUG_ASSERT(ret == 1);
      if (!capture_dir.empty()) { rdoc->SetCaptureFilePathTemplate(capture_dir.c_str()); }
    }
  }
#else
...
inline uint32_t get_rdoc_num_captures() { return rdoc->GetNumCaptures(); }
