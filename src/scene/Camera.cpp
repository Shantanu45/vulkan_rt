...
namespace {
  ...
  Vec3 add(Vec3 lhs, Vec3 rhs) { return Vec3{ lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z }; }

  Vec3 scale(Vec3 vector, float scalar) { return Vec3{ vector.x * scalar, vector.y * scalar, vector.z * scalar }; }
...
{
  if (width > 0 && height > 0) { set_aspect_ratio(static_cast<float>(width) / static_cast<float>(height)); }
}
...
float Camera::yaw_degrees() const { return yaw_degrees_; }

float Camera::pitch_degrees() const { return pitch_degrees_; }

float Camera::vertical_fov_degrees() const { return vertical_fov_degrees_; }
...
Vec3 Camera::right_direction() const { return normalize(cross(view_direction(), world_up)); }

Vec3 Camera::up_direction() const { return normalize(cross(right_direction(), view_direction())); }
}// namespace vulkan_rt::scene
